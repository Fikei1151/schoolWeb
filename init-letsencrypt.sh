...
docker-compose run --rm certbot sh -c "echo 'nameserver 8.8.8.8' > /etc/resolv.conf && echo 'nameserver 8.8.4.4' >> /etc/resolv.conf && certonly --webroot -w /var/www/certbot $staging_arg --dry-run -v --email $email -d ${domains[0]} -d ${domains[1]} --rsa-key-size $rsa_key_size --agree-tos --no-eff-email --force-renewal" || { 
...
if [ $staging = "0" ]; then
  docker-compose run --rm certbot sh -c "echo 'nameserver 8.8.8.8' > /etc/resolv.conf && echo 'nameserver 8.8.4.4' >> /etc/resolv.conf && certonly --webroot -w /var/www/certbot --email $email -d ${domains[0]} -d ${domains[1]} --rsa-key-size $rsa_key_size --agree-tos --no-eff-email --force-renewal" || { 
